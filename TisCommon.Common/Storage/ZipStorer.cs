        ...
        public List<ZipFileEntry> ReadCentralDir()
        {
            ...
            for (int pointer = 0; pointer < this.CentralDirImage.Length; )
            {
                ...
                if (comprSize == 0xffffffff ||
                   fileSize == 0xffffffff)
                {
                    int fileSizeOff = filenameSize + 50;
                    fileSize = BitConverter.ToUInt32(CentralDirImage, pointer + fileSizeOff);
                    int compOff = fileSizeOff + 8;
                    comprSize = BitConverter.ToUInt32(CentralDirImage, pointer + compOff);
